#ifndef __AR_DRIVER_AR_QSPI_H
#define __AR_DRIVER_AR_QSPI_H
...
uint8_t ar_qspi_enable_defaultInstruct(void);
uint8_t ar_qspi_update_instruct(ENUM_QUAD_SPI_INSTR_ID instr_id, uint32_t cmd_h, uint32_t cmd_l);
uint8_t ar_qspi_read_instruct(ENUM_QUAD_SPI_INSTR_ID instr_id, uint32_t * cmd_h, uint32_t * cmd_l);

uint8_t ar_qspi_write_enable(void);
uint8_t ar_qspi_write_disable(void);

uint8_t ar_qspi_check_busy(void);

uint8_t ar_qspi_erase_block(uint32_t flash_blk_st_addr);
uint8_t ar_qspi_erase_sector(uint32_t flash_sect_st_addr);
uint8_t ar_qspi_erase_chip(void);

uint8_t ar_qspi_write_byte(uint32_t flash_addr, uint8_t value);
uint8_t ar_qspi_write_halfword(uint32_t flash_addr, uint16_t value);
uint8_t ar_qspi_write_word(uint32_t flash_addr, uint32_t value);
uint8_t ar_qspi_write_block_by_byte(uint32_t flash_blk_st_addr, uint8_t* blk_val_table, uint32_t byte_size);
uint8_t ar_qspi_write_block_by_halfword(uint32_t flash_blk_st_addr, uint16_t* blk_val_table, uint32_t halfword_size);
uint8_t ar_qspi_write_block_by_word(uint32_t flash_blk_st_addr, uint32_t* blk_val_table, uint32_t word_size);

uint8_t ar_qspi_read_byte(uint32_t flash_addr, uint8_t* value_ptr);
uint8_t ar_qspi_read_halfword(uint32_t flash_addr, uint16_t* value_ptr);
uint8_t ar_qspi_read_word(uint32_t flash_addr, uint32_t* value_ptr);
void    ar_qspi_read_block_by_byte(uint32_t flash_blk_st_addr, uint8_t* blk_val_table, uint32_t byte_size);

uint8_t ar_qspi_set_speed(ENUM_QUAD_SPI_SPEED speed);

uint32_t ar_qspi_read_reg1_status(void);
uint32_t ar_qspi_read_reg2_status(void);

void    ar_qspi_set_reg1_status(uint8_t flag);
void    ar_qspi_set_reg2_status(uint8_t flag);

void    ar_qspi_read_manudeviceid(uint8_t* data_buf, uint8_t buflen);
void    ar_qspi_read_jedecid(uint8_t* data_buf, uint8_t buflen);
...
#endif
#endif /* __ASSEMBLY__ */
#endif /* __AR_DRIVER_AR8020_SPI_H */
